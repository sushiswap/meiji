...
contract VotingEscrow is Owned, ReentrancyGuard, IVotingEscrow {
    ...
    uint256 internal constant MAXTIME = 4 * 365 * 8640; // 4 years
    ...
    function increaseAmountFor(address _addr, uint256 _value) external override nonReentrant assertNotContract {
        ...
        _depositFor(_addr, _value, 0, _locked, INCREASE_LOCK_AMOUNT);
    }
    ...
    function increaseUnlockTime(uint256 _duration) external override nonReentrant assertNotContract {
        ...
        require(_locked.end > block.timestamp, "VE: LOCK_EXPIRED");
        require(_locked.amount > 0, "VE: LOCK_NOT_FOUND");
        require(unlock_time >= _locked.end + WEEK, "VE: UNLOCK_TIME_TOO_EARLY");
        require(unlock_time <= block.timestamp + MAXTIME, "VE: UNLOCK_TIME_TOO_LATE");
